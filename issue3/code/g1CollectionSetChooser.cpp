...
class G1BuildCandidateRegionsTask : public AbstractGangTask {
  ...
  public:
    ...
    // 如果当前区域 r 正在作为老年代 GC 分配区域使用，它应该被跳过，不能被纳入收集集合（CSet），因为在完全填满之前不应考虑这些区域。
      ...
      // 对于巨型区域（humongous regions），保留它们的 remembered sets。对于其他普通的老年代区域，清除它们的 remembered sets。
...
// 是否将当前区域加入到回收集中，应该是src/hotspot/share/gc/g1/g1CollectionSetCandidates.cpp文件中进行调用
bool G1CollectionSetChooser::should_add(HeapRegion* hr) {
// 不是新生代的region，并且当前region存活的数据大小live_bytes小于设置的G1MixedGCLiveThresholdPercent
  return !hr->is_young() &&
         !hr->is_pinned() &&
