...
// 这里返回的是 G1CollectorState对象，_g1h->collector_state()在src/hotspot/share/gc/g1/g1CollectedHeap.hpp；
//
...
void G1Policy::record_total_pause_time_ms(double pause_time_ms) const {
    phase_times()->record_cur_collection_total_sec(pause_time_ms);
}
...
// _ihop_control是G1IHOPControl的对象，查看src/hotspot/share/gc/g1/g1IHOPControl.cpp
  size_t marking_initiating_used_threshold = _ihop_control->get_conc_mark_start_threshold();

// _g1h是G1CollectedHeap的对象，这里得到非新生代的堆使用大小，以字节为单位。该方法在src/hotspot/share/gc/g1/g1CollectedHeap.hpp，这里可以说明IHOP参数是针对老年代的
  ...
  if (marking_request_bytes > marking_initiating_used_threshold) {
    // 调用的是src/hotspot/share/gc/g1/g1CollectorState.hpp里面的方法
    // 如果收集器的状态处于Young GC并且不处于 Mixed GC之前的最后一次 Young GC
...
void G1Policy::record_collection_pause_end(double pause_time_ms, bool concurrent_operation_is_full_mark) {
  ...
  // 设置GC停顿的类型， src/hotspot/share/gc/g1/g1CollectorState.cpp
  ...
  // 如果属于 ConcurrentStartMarkGC或者ConcurrentStartUndoGC类型的GC
...
// _analytics->prev_collection_pause_end_ms()获取上一次进入这个函数时记录的end_time_sec，这里其实就是整个程序的运行时间
  ...
  // 记录当前的停顿，里面会记录_analytics的prev_collection_pause_end_ms()
  record_pause(this_pause, start_time_sec, end_time_sec);

    // 如果是最新的一次young gc造成的停顿
  ...
  if (update_stats) {
    ...
    // [0.193s][info ][gc,phases     ] GC(0)   Other: 2.4ms这里的时间？(这里是记录除了清理CSet和候选CSet,以及free region的时间)
    _analytics->report_constant_other_time_ms(constant_other_time_ms(pause_time_ms));

    // 记录停顿的总时间
    record_total_pause_time_ms(pause_time_ms);
  ...
  // 如果在 G1 的 periodic collection 收集，将不会更新IHOP。就是处于G1的Mixed GC的阶段中，是不会进行更新IHOP的，我们知道一轮Mixed GC是分为G1MixedGCCountTarget次进行的
...
TruncatedSeq* G1Policy::candidates_cset_seq() {
    return _candidates_cset_seq;
}


TruncatedSeq* G1Policy::region_cset_seq() {
    return _region_cset_seq;
}

void G1Policy::adjust_parameters_for_pause_time(double current_pause_time, double target_pause_time) {
//    double pause_time_ratio = current_pause_time / target_pause_time;
//
//    if (pause_time_ratio > 1.1) {  // 当前停顿时间超过目标的10%
//        // 减少收集集合大小以缩短停顿时间
//        G1OldCSetRegionThresholdPercent = MAX2(G1OldCSetRegionThresholdPercent * 0.9, (uintx)5);
//        G1MixedGCCountTarget = MIN2(G1MixedGCCountTarget + 1, (uintx)10);
//    } else if (pause_time_ratio < 0.9) {  // 当前停顿时间低于目标的10%
//        // 增加收集集合大小以提高效率
//        G1OldCSetRegionThresholdPercent = MIN2(G1OldCSetRegionThresholdPercent * 1.1, (uintx)20);
//        G1MixedGCCountTarget = MAX2(G1MixedGCCountTarget - 1, (uintx)5);
//    }
//
//    // 调整G1MixedGCLiveThresholdPercent
//    if (pause_time_ratio > 1.2) {
//        G1MixedGCLiveThresholdPercent = MAX2(G1MixedGCLiveThresholdPercent - 5, (uintx)25);
//    } else if (pause_time_ratio < 0.8) {
//        G1MixedGCLiveThresholdPercent = MIN2(G1MixedGCLiveThresholdPercent + 5, (uintx)90);
//    }
}

// 在这个函数里面，将会更新G1OldCSetRegionThresholdPercent，G1MixedGCLiveThresholdPercent和 G1MixedGCCountTarget
...
//
//    // 这里是加入到候选集合的比例
////    uint avg_cset_rate = ((candidates_region_num * 100) / all_region_num);
...
//        G1MixedGCCountTarget = MIN2( G1MixedGCCountTarget + add,(uintx)10);
//    } else {
//        rate = (1.0 * diff_candidates) / history_avg_candidates;
//        uintx reduce = (uintx) rate * G1MixedGCCountTarget;
//        G1MixedGCCountTarget = MAX2( G1MixedGCCountTarget - reduce,(uintx)6);
//    }
//
...
//        G1OldCSetRegionThresholdPercent = MAX2( G1OldCSetRegionThresholdPercent - reduce,(uintx)6);
//    } else {
//        rate = (1.0 * diff_region) / history_avg_region_num;
//        uintx add = (uintx) rate * G1OldCSetRegionThresholdPercent;
//        G1OldCSetRegionThresholdPercent = MIN2( G1OldCSetRegionThresholdPercent + add,(uintx)18);
//    }
...
//    }
//
//    // 更新G1OldCSetRegionThresholdPercent
//    int diff_region = all_region_num - history_avg_region_num;
//    if (diff_region < 0) {
...
//    }
        ...
        double last_avg_total_pause_time = last_avg_total_pause_time_ms();
        record_last_avg_total_pause_time_ms(predict_pause_time);
        double pause_factor = predict_pause_time / last_avg_total_pause_time;
        pause_factor = MIN2(MAX2(pause_factor, 0.2), 2.0);
        ...
        if (diff_candidates > 0) {
            double rate = (1.0 * diff_candidates) / (history_avg_candidates);
            uintx add = (uintx)(rate * pause_factor  * G1MixedGCCountTarget);
            G1MixedGCCountTarget = MIN2((uintx)(G1MixedGCCountTarget * (10 - smoothing_factor) +  (G1MixedGCCountTarget + add) * smoothing_factor) / 10, (uintx)14);
            add = (uintx)(rate * pause_factor * G1OldCSetRegionThresholdPercent);
            G1OldCSetRegionThresholdPercent = MIN2((uintx)(G1OldCSetRegionThresholdPercent * (10 - smoothing_factor) +  (G1OldCSetRegionThresholdPercent + add) * smoothing_factor) / 10 , (uintx)16);
        } else {
            double rate = (-1.0 * diff_candidates) / (history_avg_candidates);
            uintx reduce = (uintx)(rate * pause_factor * G1MixedGCCountTarget);
            ...
            reduce = (uintx)(rate * pause_factor * G1OldCSetRegionThresholdPercent);
        ...
        log_debug(gc, ergo, cset)("paramters predict_pause_time: %1.2fms, last_avg_total_pause_time: %1.2fms, pause_factor: %1.2fms, candidates_region_num: %lu, history_avg_candidates: %1.2fms",
                                      predict_pause_time, last_avg_total_pause_time, pause_factor, candidates_region_num, history_avg_candidates);
    }
}


//// 在这个函数里面，将会更新G1OldCSetRegionThresholdPercent，G1MixedGCLiveThresholdPercent和G1MixedGCCountTarget
//void G1Policy::update_other_g1_params(size_t candidates_region_num) {
//
//    // 获取堆的全部region数量
//    const G1CollectedHeap* g1h = G1CollectedHeap::heap();
...
////        }
////
////     获取堆栈的信息，通过堆栈中对象存活比例的 G1MixedGCCountTarget*G1OldCSetRegionThresholdPercent个数，是否
////     _g1h->heap();
////     HeapRegionClosure rli();
////
////    if (predict_pause_time < last_avg_total_pause_time) {
////        G1MixedGCCountTarget = MAX2(G1MixedGCCountTarget - 1, (uintx)4);
////        G1OldCSetRegionThresholdPercent = MIN2( G1OldCSetRegionThresholdPercent + 3, (uintx)25);
//////        G1MixedGCLiveThresholdPercent = MIN2( G1MixedGCLiveThresholdPercent + 5, (uintx)95);
////    } else {
...
//////        G1MixedGCLiveThresholdPercent = MAX2( G1MixedGCLiveThresholdPercent - 5, (uintx)60);
////    }
//}

// 这个函数的作用就是往g1HOPControl.cpp中的_marking_times_s队列中添加数据
...
// 这个预测函数在src/hotspot/share/gc/g1/g1CollectionSet.cpp的finalize_young_part()函数中调用
...
// 用于计算 G1 GC 在混合垃圾收集(Mixed GC)期间每次GC最小的老年代集合大小。
uint G1Policy::calc_min_old_cset_length(G1CollectionSetCandidates* candidates) const {
// 最小老年代集合大小是基于每个 GC 周期后希望执行的混合 GC 的最大数量，即使某些老年代区域回收代价较高，仍然需要将它们加入收集集合，以确保在一定数量的混合 GC 内遍历所有老年代区域。
  // The min old CSet region bound is based on the maximum desired
  // number of mixed GCs after a cycle. I.e., even if some old regions
  // look expensive, we should add them to the CSet anyway to make
  // sure we go through the available old regions in no more than the
  // maximum desired number of mixed GCs.
  // 这里进一步说明计算是基于初始加入 CSet 候选集合中的标记区域数量，而不是剩余的区域数量，这样在每个混合 GC 中，结果会保持一致。
...
// 获取候选的老年代区域的数量
  ...
  // MAX2取最大的值，G1MixedGCCountTarget默认为8，定义了变量 gc_num，表示期望的混合 GC 次数。
  const size_t gc_num = (size_t) MAX2(G1MixedGCCountTarget, (uintx) 1);
  // 每次混合GC至少需要手机的老年代区域数量
...
// 计算混合GC阶段每次GC最大的老年代集合大小
uint G1Policy::calc_max_old_cset_length() const {
// 最大老年代回收集是基于堆大小的某个百分比作为阈值，即不管实际有多少老年代区域，可以加入到回收集中的老年代数量由堆大小的百分比决定
  ...
  //G1OldCSetRegionThresholdPercent是一个整数，要让其表示百分比，就得除100
  // 在这里更新这个参数

  const size_t perc = (size_t) G1OldCSetRegionThresholdPercent;
  // 每次收集时 CSet 的region数量
...
//void G1Policy::updateCountAndCSetThreshold(G1CollectionSetCandidates* candidates) const {
//    // 获取候选的老年代区域的数量
//    const size_t region_num = candidates->num_regions();
//
//    // 获取堆的全部region数量
...
/*
time_remaining_ms：剩余的可用于垃圾收集的时间（以毫秒为单位）
num_initial_regions和num_optional_regions：存储计算出的初始和可选的老年代区域数
*/
// 会在src/hotspot/share/gc/g1/g1CollectionSet.cpp中的finalize_old_part方法使用
void G1Policy::calculate_old_collection_set_regions(G1CollectionSetCandidates* candidates,
  ...
  // 计算可选区域的时间阈值，这个阈值决定了加入可选老年代区域时所能使用的最大时间，optional_prediction_fraction()返回0.2
...
//   }
  log_debug(gc, ergo, cset)("111111 paramters G1MixedGCCountTarget: %lu, G1OldCSetRegionThresholdPercent: %lu, G1MixedGCLiveThresholdPercent: %lu",
  ...
  //  获取候选区域集合的当前索引，candidate_idx 表示当前处理的候选区域的位置。
  uint candidate_idx = candidates->cur_idx();

  // 输出debug级别的日志：记录开始选择老年代区域的日志，输出最小和最大区域数、剩余时间和可选区域的时间阈值
  ...
  log_debug(gc, ergo, cset)("222222 paramters G1MixedGCCountTarget: %lu, G1OldCSetRegionThresholdPercent: %lu, G1MixedGCLiveThresholdPercent: %lu",
                              G1MixedGCCountTarget, G1OldCSetRegionThresholdPercent, G1MixedGCLiveThresholdPercent);

  // 获取候选区域的当前 HeapRegion，并进入一个循环，直到没有更多的候选区域（即 hr == NULL 时）。
...
// G1 垃圾收集器的混合阶段中计算可选的老年代收集区域，依据剩余的收集时间和候选区域的集合决定加入多少个区域
/*
max_optional_regions：可选的最大区域数
time_remaining_ms：剩余时间
num_optional_regions：最终计算出的可选区域数
*/
void G1Policy::calculate_optional_collection_set_regions(G1CollectionSetCandidates* candidates,
  ...
  while (num_optional_regions < max_optional_regions) {
    ...
    // 预测处理当前区域所耗费的时间，并添加到预测的总消耗时间里
    prediction_ms += predict_region_total_time_ms(r, false);

    // 如果预测的时间大于剩余的时间（剩余的时间其实是用户设置的GC最大的停顿时间）
